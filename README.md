...
Players sign in with just a nickname and pick an avatar, and a player who disconnects
unexpectedly (dropped network, closed tab, killed browser) keeps their seat and score and
resumes exactly where they left off when they reconnect. Installable as a PWA (Add to Home
Screen) on phones.
...
Implemented: room creation/join by code (single-click copy on the room code), host-configurable
settings (players, draw time, rounds, word count, hints, game mode incl. hidden word length,
custom words, private/public), fair turn rotation (everyone draws once per round), word choice
...
and players who've already guessed), scoring, like/dislike reactions on the drawing,
round/game-end scoreboards, play again, avatar customization at login, click-to-kick voting,
nickname (guest) auth, disconnect-and-resume, a fixed-height scrollable chat with a character
counter, and basic abuse protection (server-side input validation and rate limiting on auth,
chat, and room churn). The game returns to the lobby if fewer than two players remain.
